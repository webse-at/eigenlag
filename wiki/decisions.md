...
## ADR-023 — Englisch ist Report-Default, Deutsch bleibt vollwertig unter `--lang de`

**Status:** entschieden, 2026-07-15 (Session 011, David hat den Veto-Punkt bestätigt: MIT-Lizenz, Englisch-Default)

**Kontext:** Bis Session 010 war die Nutzer-Ausgabe der CLI durchgehend deutsch. Der Feedback-Meilenstein (`wiki/roadmap.md`) läuft über r/dataengineering, den Airflow-Slack und Wikimedia — die Zielgruppe liest englisch. Wer dort `pipx install` ausführt und einen deutschen Report bekommt, ist weg, bevor er die erste Zahl liest.

**Entscheidung:**
1. **Englisch wird Report-Default, `--lang de` bleibt vollwertig und getestet.** Zwei Nachrichten-Kataloge als schlichte dicts in `eigenlag/messages.py`, keine i18n-Bibliothek (kein gettext, kein Babel). Ein Test (`messages_test.py`) erzwingt Katalog-Vollständigkeit: jeder Key existiert in beiden Sprachen, kein stiller Fallback. Deutsch bleibt Davids Referenz-Formulierung und die Abnahme-Sprache.
2. **`--json` ist über beide Sprachen byte-identisch.** Die Sprache berührt nur `render()`/`render_check()`, nie `compose()`/`compose_check()`. Die JSON-Werte sind deutsch eingefroren (auch die Prosa-Felder wie `gruende`, `behebung`, `szenario`). Zwei Tests pinnen das (`i18n_test.py`).
3. **JSON-Keys bleiben eingefroren** (auch die deutschen: `auf_kreis`, `gruende` …). Sie sind seit Session 010 die Gate-Schnittstelle; ein Rename wäre ein Breaking Change ohne Nutzen.
4. **argparse-Hilfe, CLI-Fehlermeldungen und die Quellen-Beschriftungen sind einsprachig englisch.** Sie tragen keinen `--lang`-Kontext (der Flag wird erst geparst); halb übersetzt wäre unehrlicher als konsequent englisch.
5. Fachbegriffe bleiben unübersetzt: λ, Cross-Run, DAG, Task, Makespan. Glossar sonst: Takt → *period*, Taktzeit → *cycle time*, kritischer Kreis → *critical cycle*, Reserve → *headroom*, Task-Einheiten → *task units*.
6. Version bleibt `0.1.0`, Lizenz **MIT** (Davids Entscheidung), kein PyPI-Upload in dieser Session.

**Ein bewusster Schnitt an `compose()`:** Die Spec verlangte „compose() wird nicht angefasst". In `compose()` steckte aber deutscher Prosa-Text in generierten Feldern (What-if-Szenario-Labels, Gate-Fail-Gründe, Behebungs-Hinweis). Ein korrekter englischer Report kann diese Felder nicht verbatim lesen. Auflösung: die bestehenden Keys und ihre deutschen Werte bleiben unverändert (Gate liest weiter, DE-Tests grün), es kommen **sprachneutrale strukturierte Begleitfelder** dazu (`art`/`task`/`src`/`dst` je What-if-Zeile, `gruende_codes`/`behebung_code`/`hinweis_codes` im Gate), aus denen `render(…, lang)` den Text pro Sprache baut. Damit bleibt `--json` sprachunabhängig und byte-identisch, und „compose nicht anfassen" gilt als „`--json` bleibt stabil", nicht als „null neue Felder".

**Geflaggte Grenze (Folge-Ticket):** Vollständig strukturierte Warnungen gibt es noch nicht. Die terse Diagnose-Details des sprachneutralen Parser-/Dauern-Layers (z. B. „no measurement, 300 s", die `edge_dropped`-Gründe) wurden für diese Session **einsprachig englisch** gezogen — englisch ist die Default-Ausgabe und diese Strings sind Diagnostik, keine der in der Spec gelisteten heikle Formulierungen. Der deutsche Report zeigt diese Details darum ebenfalls englisch; die Report-Kernprosa (Urteil, Kreis, Monte Carlo, What-if, Sensor-Texte, Modellgrenzen) ist über den Katalog voll zweisprachig. Voll bilinguale Details bräuchten strukturierte Warnungen (`kind` + Parameter statt fertigem `detail`-String) — sauberes Folge-Ticket, nicht Teil von 011.

## ADR-024 — Aus der Diagnose wird das Produkt: der Beschleunigungsplan

**Status:** entschieden, 2026-07-16 (Session 012).

**Kontext:** Nach Phase 1 hat sich der Wert verschoben (`positioning.md`, Zwischenbewertung): weg vom λ-Wert selbst, hin zur Handlungsorientierung — der Unterscheidung Struktur- vs. Kapazitätsproblem und der Reserve, von der ein Team nicht weiß, dass es sie hat. Der Report sagte bis 011 „hier ist deine Grenze und wer schuld ist". Das ist Diagnose. Das Produkt sagt: „deine Pipeline könnte alle X laufen statt alle Y; hier ist die Änderung, die den Unterschied kauft, und was sie bringt."

**Entscheidung:**

1. **Der Beschleunigungsplan ersetzt die What-if-Sektion im Report** (Reihenfolge: Urteil → kritischer Kreis → Plan → Monte Carlo → Warnungen). Er rechnet dieselben Aktionen wie das What-if-Ranking (Kreis-Task halbieren, Cross-Kante entfernen, plus `--what-if`-Nutzer-Szenarien), formuliert sie aber als unbeanspruchte Reserve. `eigenlag/plan.py::build_plan` reichert die What-if-Zeilen an; `report.py::_plan_text` rendert. Der `what_if`-JSON-Key bleibt eingefroren daneben (das Gate liest ihn), der `plan`-Key kommt **additiv** dazu (ADR-023-Politik).

2. **Zwei Gewinn-Formulierungen, exakt definiert.** *Instabil* (λ > T): eine Aktion macht den Takt tragfähig genau dann, wenn λ_neu < T; beziffert wird die weggeräumte Drift (λ − T pro Lauf). Rettet keine Einzel-Aktion, rechnet der Plan die Paare der drei wirksamsten Aktionen (drei Paare, keine Kombinatorik-Explosion). *Stabil* (λ < T): der Gewinn ist Headroom — λ ist die Untergrenze, ein Takt von λ statt T liefe (86400/λ − 86400/T) mal pro Tag öfter und hielte die Daten **bis zu** (T − λ) frischer. Das „bis zu" ist Pflicht, die Fußzeile sagt, dass λ eine Untergrenze ohne Betriebsreserve ist. Es wird **kein** konkreter neuer Schedule empfohlen (Regel 2: keine erfundene Marge).

3. **Kein `--value-per-hour`-Flag, keine Euro-Rechnung im Tool.** Das Tool liefert die physikalischen Gewinnzahlen (Läufe pro Tag, Frische-Delta, weggeräumte Drift); die Übersetzung in Geld machen die Marketing-Texte in Prosa. Eine Pseudo-Währung wie „€ pro Stunde Datenfrische" kippt die Glaubwürdigkeit bei einer skeptischen Zielgruppe.

4. **Der Behebungs-Katalog ist Muster-Wissen, kein Orakel.** Je Kanten-Art (A `depends_on_past`, B `wait_for_downstream`, C Sensor mit `execution_delta`, D `include_prior_dates`, G `max_active_runs=1`, dbt-E `is_incremental`) ein Muster in beiden Sprachen (`messages.py`, `plan_fix_*`), plus ein generisches für die Task-Halbierung. Formuliert als „üblicher Weg" / „commonly resolved by …", nie als Garantie. Die Kanten-Art kommt aus der Signal-Herkunft der geparsten DAGs; ein direkt gebautes Pipeline-Objekt (die Demo) trägt kein Signal und bekommt keinen Katalog-Text (kein erfundenes Detailwissen über fremde Tasks). Der Katalog ist auch für D und E vollständig, obwohl der Airflow-Parser für sie heute keine Kante erzeugt — er ist Muster-Wissen, das greift, sobald die Kante existiert. Ein Test erzwingt Vollständigkeit (jede Kanten-Art × beide Sprachen).

**Verifiziert (Belege in `scan/012_plan/`):** Die Demo-Pipeline (Prototyp-Ground-Truth, λ = 4.40 h, T = 3.0 h, instabil) ist der Pin und zugleich das Marketing-Artefakt: die kostenlose Architektur-Änderung (Quality-Gate-Kante `monitor → core` entfernt) bringt λ_neu = 2.50 h < T und macht den Takt tragfähig (−43,18 %, räumt 84 min/Lauf Drift weg), das GPU-Upgrade (`retrain` halbiert → 3.60 h) rettet ihn nicht. Das Flaggschiff `load_data_wikiviews` (stabil, λ = 600 s, T = 3600 s) zeigt Headroom: 120 Läufe/Tag mehr, bis zu 50 min frischer. Ein synthetischer Zwei-Loop-Fall zeigt die Paar-Rechnung.
